...
class MainState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Process the event from the current player, if possible.
        """
    ...
    def update(self) -> GameState:
        ...
        Return the GameOverState iff the current turn has reached the limit of
        maximum turns. Return self if no move was made or the move was not
        valid. Return the AnimateState iff a valid move was made to animate
        the move.
        """
    ...
    def render(self, renderer: Renderer) -> None:
        """Render the current status and selected block
        (if any) of the game onto the screen using <renderer>.
        """
...
class AnimateMoveState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Returns nothing as <event> is ignored."""
        return  # Ignore the event

    def update(self) -> GameState:
        """Return the last GameState when the animation is complete. Return
        self otherwise."""
    ...
    def render(self, renderer: Renderer) -> None:
        """Render the outline around the selected block, the image representing
        the move, and update the status message based on the action being
        performed onto the screen using <renderer>.
        """
...
class GameOverState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Returns nothing as <event> is ignored when the game is over."""
        # Simply ignore the event
        return

    def update(self) -> GameState:
        """Return self and change nothing."""
        # Nothing to change
        return self

    def render(self, renderer: Renderer) -> None:
        """Render the final scores of the players and the winner onto the screen
        using <renderer>.
        """
        x = 10
        y = 10
